...
contract DeployLimitOrder is Script {
    ...
    function run() external {
        ...
        bytes memory data =
            abi.encodeWithSelector(SynthetixLimitOrdersV3.initialize.selector, OWNER, POLYLIST, PYTHNODE, PERP_MARKET);
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(address(synthetixLimitOrders), ADMIN, data);

        SynthetixLimitOrdersV3 target = SynthetixLimitOrdersV3(payable(address(proxy)));

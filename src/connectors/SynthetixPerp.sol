...
contract SynthetixPerpConnector is BaseConnector {
    ...
    IDynamicGasFees public constant dynamicGas =
        IDynamicGasFees(0xF4bc5588aAB8CBB412baDd3674094ECF808286f6);
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        ...
        uint256 desiredPrice = sizeDelta > 0
            ? price.mulWadDown(WAD + slippage)
            : price.mulWadDown(WAD - slippage);

        IPerpMarket(market).submitOffchainDelayedOrderWithTracking(
            sizeDelta,
            desiredPrice,
            "polynomial"
        );
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        ...
        IPerpMarket(market).submitCloseOffchainDelayedOrderWithTracking(
            desiredPrice,
            "polynomial"
        );
    ...
    function execute(
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        IPerpMarket(market).executeOffchainDelayedOrder{value: msg.value}(
            address(this),
            priceUpdateData
        );
